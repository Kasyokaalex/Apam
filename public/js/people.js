$(document).ready(function () {
  ...
  $("#btn-add-row").click(function () {
    ...
    if (rowCount > 1) {
      $("#tbl-income-sources tbody").append(
        "<tr>" + $("#tbl-income-sources tr:last").html() + "</tr>"
      );
    } else {
      $("#tbl-income-sources tbody").append(
        "<tr><td><input type='checkbox' class='select_' /></td><td><input type='text' class='form-control' name='sources[]' /></td><td><input type='number' class='form-control' name='values[]' /></td></tr>"
      );
    }
  });
  ...
  var uploader_a = new plupload.Uploader({
    ...
    filters: {
      ...
      mime_types: [
        {
          title: "All files",
          extensions: "jpg,gif,png,xls,xlsx,csv,doc,docx,pdf",
        },
      ],
    },
    init: {
  ...
  uploader_a.bind("FileUploaded", function (upldr, file, object) {
    ...
    $("#filelist_a").append(
      '<li><a href="uploads/tenant-' +
        ...
        '"><img src="' +
        myData.icon +
        '" /></a><span class="close remove-file" data-file-id="' +
        myData.id +
        '" title="Remove this file"><i class="fa fa-times-circle"></i></span></li>'
    );
  });
